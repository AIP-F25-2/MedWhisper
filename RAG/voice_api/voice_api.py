# file: RAG/voice_api/voice_api.py
...
class TextQARequest(BaseModel):
    ...
    # REQUIRED – no default. Frontend/token must provide role.
    user_role: str
...
async def text_qa(payload: TextQARequest = Body(...)) -> Dict[str, Any]:
...

    {
      ...
      "user_role": "clinician"  // or 'student', 'nurse', 'admin', etc. – REQUIRED
    }
    """
...
async def voice_qa(
    ...
    # REQUIRED – no default. Frontend must send user_role.
    user_role: str = Form(...),
) -> Dict[str, Any]:
    """
    Voice QA:
    ...
    try:
        # ✅ async file I/O to satisfy SonarCloud & avoid blocking event loop
