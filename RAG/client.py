...
class RetrievalClient:
    def __init__(self, base_url: str | None = None, timeout: float = 20.0):
        # read env here (runtime), not at import time
        env_base = os.getenv("RETRIEVAL_BASE_URL", "http://127.0.0.1:8001")
    ...
    def candidates(self, q: str, k: int = 12, alpha: float = 0.65, blend_ce: float = 1.0) -> List[Dict[str, Any]]:
        try:
            data = self._post("/search_rerank", {"q": q, "k": k, "alpha": alpha, "blend_ce": blend_ce, "filters": None})
