...
async def chat_voice(audio_file: UploadFile = File(...), sender: str = Form(...)):
    ...
    try:
        ...
        try:
            ...
            # Try to load the audio file in various formats
            try:
                ...
                # If pydub fails (no ffmpeg), try direct processing
                logger.warning("pydub conversion failed, trying direct speech recognition...")
                wav_path = tmp_file_path
            
        except Exception as audio_error:
            logger.warning(f"Audio conversion failed: {audio_error}, trying direct processing...")
            wav_path = tmp_file_path
        ...
        try:
            ...
            logger.error(f"Speech recognition file error: {sr_error}")
            # Clean up temp files
            try:
                os.unlink(tmp_file_path)
                if wav_path != tmp_file_path and os.path.exists(wav_path):
                    os.unlink(wav_path)
            except:
                pass
            
            # Provide helpful fallback response instead of error
            fallback_reply = """🎤 **Voice Message Received**

I received your voice message, but I'm having trouble processing the audio format right now. Here's some general medical guidance:

**Common Health Questions:**
• **Symptoms**: Document when they started, severity, and any triggers
• **Medications**: Keep an updated list of all prescriptions and supplements  
• **Emergency**: Call 911 for chest pain, difficulty breathing, or severe injuries
• **Routine Care**: Schedule regular check-ups with your healthcare provider

**For Voice Messages:**
• Try speaking clearly and slowly
• Ensure good microphone quality
• Alternatively, you can type your question for immediate assistance

⚠️ **Medical Disclaimer:** This is general information only. For specific medical concerns, always consult with a qualified healthcare professional."""
            
            return JSONResponse(content={"text": fallback_reply})
...
⚠️ **Important:** This is general information only. For specific medical concerns, always consult with a qualified healthcare professional."""
        ...
        try:
            os.unlink(tmp_file_path)
            if 'wav_path' in locals() and wav_path != tmp_file_path and os.path.exists(wav_path):
                os.unlink(wav_path)
        except:
            pass
