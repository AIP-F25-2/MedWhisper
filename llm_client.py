...
- Recommend further investigation when appropriate"""
        ...
        if use_fallback_for_general and len(context_docs) == 0:
            ...
            general_medical_prompt = f"""You are ClinicalCamel, a medical AI assistant extensively trained on clinical literature, medical textbooks, and evidence-based medicine.

MEDICAL QUESTION: {query}

INSTRUCTIONS:
Provide a comprehensive, accurate, evidence-based medical answer using your clinical training. Structure your response professionally:

1. **Definition/Overview** - What is this condition/topic?
2. **Key Information** - Main facts, mechanisms, or explanations
3. **Clinical Details** - Symptoms, presentations, characteristics, or procedures
4. **Important Considerations** - Clinical significance, warnings, or guidelines
5. **Standard Treatment/Management** (if applicable)

Use clear medical terminology while remaining understandable. Be thorough and specific.

Note: This is general medical knowledge based on clinical training, not patient-specific information.

COMPREHENSIVE ANSWER:"""
            
            response = self.generate(general_medical_prompt, temperature=0.4, max_tokens=2048)
            
            # Add note about fallback mode
            if response:
                response += "\n\n**[Response generated by ClinicalCamel - General Medical Knowledge Mode]**"
